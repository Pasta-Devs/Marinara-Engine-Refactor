...
fn authorize(state: &AppState, route: &ParsedPath, body: &Value) -> AppResult<Value> {
    ...
    Ok(
        json!({ "authUrl": format!("https://accounts.spotify.com/authorize?{params}"), "redirectUri": SPOTIFY_REDIRECT_URI }),
    )
}

async fn exchange(state: &AppState, body: Value) -> AppResult<Value> {
